...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
	...
	input_bufferA = CL_CHECK_ERR(clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_half)*NUM_DATA*NUM_DATA, NULL, &_err));
	cl_mem input_bufferB;
	input_bufferB = CL_CHECK_ERR(clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_half)*NUM_DATA*NUM_DATA, NULL, &_err));

  printf("attempting to create output buffer\n");
  fflush(stdout);
	cl_mem output_buffer;
	output_buffer = CL_CHECK_ERR(clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(cl_half)*NUM_DATA*NUM_DATA, NULL, &_err));
  ...
  fflush(stdout);
	kernel = CL_CHECK_ERR(clCreateKernel(program, "hsgemm_unroll", &_err));
  ...
  fflush(stdout);
    ...
    cl_half in = poclu_float_to_cl_half(((float)rand()/(float)(RAND_MAX)) * 10.0);
		CL_CHECK(clEnqueueWriteBuffer(queue, input_bufferA, CL_TRUE, i*sizeof(cl_half), 2, &in, 0, NULL, NULL));
    in = poclu_float_to_cl_half(((float)rand()/(float)(RAND_MAX)) * 10.0);
		CL_CHECK(clEnqueueWriteBuffer(queue, input_bufferB, CL_TRUE, i*sizeof(cl_half), 2, &in, 0, NULL, NULL));
	}

	cl_event kernel_completion;
  const size_t local_work_size[3] = { 128, 1, 1};
  //                             a_offset  
	size_t global_work_size[3] = { NUM_DATA, NUM_DATA/4, NUM_DATA/4 };
  ...
  printf("time(ns):%lg\n",elapsed);
	...
	for (int i=0; i<NUM_DATA*NUM_DATA; i++) {
		cl_half data;
		CL_CHECK(clEnqueueReadBuffer(queue, output_buffer, CL_TRUE, i*sizeof(cl_half), 2, &data, 0, NULL, NULL));
		//printf(" %f", poclu_cl_half_to_float(data));
	}
	printf("\n");
