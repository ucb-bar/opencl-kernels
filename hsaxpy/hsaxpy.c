...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
	cl_mem input_buffer;
	input_buffer = CL_CHECK_ERR(clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_half)*NUM_DATA, NULL, &_err));
  ...
  fflush(stdout);
	kernel = CL_CHECK_ERR(clCreateKernel(program, "hsaxpy", &_err));
  ...
  const size_t local_work_size[1] = { 4 };
