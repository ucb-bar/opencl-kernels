...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
	...
	input_bufferA = CL_CHECK_ERR(clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(double)*NUM_DATA*NUM_DATA, NULL, &_err));
	cl_mem input_bufferB;
	input_bufferB = CL_CHECK_ERR(clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(double)*NUM_DATA*NUM_DATA, NULL, &_err));
  ...
  fflush(stdout);
	kernel = CL_CHECK_ERR(clCreateKernel(program, "dgemm_unroll", &_err));
  ...
  fflush(stdout);
    ...
    double in = ((double)rand()/(double)(RAND_MAX)) * 100.0;
		CL_CHECK(clEnqueueWriteBuffer(queue, input_bufferA, CL_TRUE, i*sizeof(double), 8, &in, 0, NULL, NULL));
    in = ((double)rand()/(double)(RAND_MAX)) * 100.0;
		CL_CHECK(clEnqueueWriteBuffer(queue, input_bufferB, CL_TRUE, i*sizeof(double), 8, &in, 0, NULL, NULL));
	}
