...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
	cl_mem input_buffer;
	input_buffer = CL_CHECK_ERR(clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_half)*NUM_DATA*NUM_DATA, NULL, &_err));
	cl_mem mask;
  ...
  float m2 = ((float)rand()/(float)(RAND_MAX)) * 1.0;
  float m3 = ((float)rand()/(float)(RAND_MAX)) * 1.0;
  float m4 = ((float)rand()/(float)(RAND_MAX)) * 1.0;
  float m5 = ((float)rand()/(float)(RAND_MAX)) * 1.0;
  float m6 = ((float)rand()/(float)(RAND_MAX)) * 1.0;
  float m7 = ((float)rand()/(float)(RAND_MAX)) * 1.0;
  float m8 = ((float)rand()/(float)(RAND_MAX)) * 1.0;

  printf("attempting to create kernel\n");
  fflush(stdout);
	kernel = CL_CHECK_ERR(clCreateKernel(program, "mask_hsfilter_unroll", &_err));
  ...
  fflush(stdout);
	...
	size_t global_work_size[2] = { NUM_DATA - 2, (NUM_DATA - 2)/2};//avoid the edges
  const size_t local_work_size[2] = { 128, 1 };
